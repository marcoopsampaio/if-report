...
There is an auxiliary file that I asked from FCT with the panel information for each candidate. It is located at the relative path
```{r}
  path_panel_info_2013 <- "raw_data/data_email_fct_panels/ListaCandidaturas_IF2013.xlsx"
```
...
```
**Panel info**
Before exporting we will add the panel information from the auxiliary FCT file

```{r}
  data_which_panel_2013 <- read.xlsx(path_panel_info_2013,sheetIndex = 1,startRow = 1)[,1:3]
  colnames(data_which_panel_2013)<-c("Ref","Name","Panel")
```
Now we convert the panel names to a Standardized notation. 
```{r}
  sort(levels(data_which_panel_2013$Panel))
  levels(data_which_panel_2013$Panel) <- c("env","eng","sci","hum","med","nat","soc")
  rownames(data_2013)<-c() #This just clears the row names
```
We remove the odd case also from the whic_panel info dataframe
```{r}
  data_which_panel_2013[data_which_panel_2013$Ref==setdiff(data_which_panel_2013$Ref,data_2013$Ref),]
  data_which_panel_2013 <- data_which_panel_2013[-91,] 
  data_which_panel_2013$Ref <- factor(data_which_panel_2013$Ref)
  data_which_panel_2013$Name <- factor(data_which_panel_2013$Name)
  rownames(data_which_panel_2013)<- c()
```
Now we check that everything is OK i.e. that the Ref,Name pairs are the same in both dataframes and ordered in the same way.
```{r}
  sum(data_which_panel_2013[,c("Ref")]!=data_2013[,c("Ref")])
  diff_names2013 <-as.character(data_which_panel_2013[,c("Name")])!=as.character(data_2013[,c("Name")])
  data.frame(data_which_panel_2013[diff_names2013,c("Name")],data_2013[diff_names2013,c("Name")])
```
We see that there are some typos in 16 names but all references match so we can just add an extra column with the panel information to the final data frame:

```{r}
  data_2013$Panel <- data_which_panel_2013$Panel
  head(data_2013)
```
...
```{r}
  path_panel_info_2014 <- "raw_data/data_email_fct_panels/ListaCandidaturas_IF2014.xlsx"
```
...
```{r}
  data_which_panel_2014 <- read.xlsx(path_panel_info_2014,sheetIndex = 1,startRow = 1)[,1:3]
  colnames(data_which_panel_2014)<-c("Ref","Name","Panel")
```
Now we convert the panel names to a Standardized notation. But before that we need to remove the candidates that are not eligible.
```{r}
  non_eligible2014<-setdiff(sort(data_which_panel_2014$Ref),sort(data_2014$Ref))
  for(item_ref in non_eligible2014){
    data_which_panel_2014 <- data_which_panel_2014[-as.integer(rownames(data_which_panel_2014[data_which_panel_2014$Ref == item_ref,])),] 
      rownames(data_which_panel_2014)<- c()
  }
  data_which_panel_2014$Ref <- factor(data_which_panel_2014$Ref)
  dim(data_which_panel_2014)
```
Now we check that everything is OK i.e. that the Ref,Name pairs are the same in both dataframes and ordered in the same way.
```{r}
  sum(data_which_panel_2014[,c("Ref")]!=data_2014[,c("Ref")])
```
Next let's check differences in cadidate names
```{r}
  diff_names2014 <-as.character(data_which_panel_2014[,c("Name")])!=as.character(data_2014[,c("Name")])
  data.frame(data_which_panel_2014[diff_names2014,c("Name")],data_2014[diff_names2014,c("Name")])
```
So all is fine. Now we can change the levels of the panel to the standard form
```{r}
  levels(data_which_panel_2014$Panel)<- c("env","eng","sci","hum","med","nat","soc")
```

Finally... we can check that the panel information that we had already worked out was correct (for the cases where we had such info):
```{r}
  pos2014_panel_webfiles <- data_2014$Panel != "empty"
  setdiff(data_2014[pos2014_panel_webfiles,"Panel"],data_which_panel_2014[pos2014_panel_webfiles,"Panel"])
```
So now we can just safely replace the Panel column
```{r}
  setdiff(data_2014$Ref,data_which_panel_2014$Ref)
  data_2014$Panel <-data_which_panel_2014$Panel
  head(data_2014)
```
...
There is an auxiliary file that I asked from FCT with the panel information for each candidate. It is located at the relative path
```{r}
  path_panel_info_2015 <- "raw_data/data_email_fct_panels/ListaCandidaturas_IF2015.xlsx"
```
...
```{r}
  data_which_panel_2015 <- read.xlsx(path_panel_info_2015,sheetIndex = 1,startRow = 1)[,1:3]
  colnames(data_which_panel_2015)<-c("Ref","Name","Panel")
```
Now we convert the panel names to a Standardized notation. But before that we need to remove the candidates that are not eligible.

```{r}
  non_eligible2015<-setdiff(sort(data_which_panel_2015$Ref),sort(data_2015$Ref))
  for(item_ref in non_eligible2015){
    data_which_panel_2015 <- data_which_panel_2015[-as.integer(rownames(data_which_panel_2015[data_which_panel_2015$Ref == item_ref,])),] 
      rownames(data_which_panel_2015)<- c()
  }
  data_which_panel_2015$Ref <- factor(data_which_panel_2015$Ref)
  dim(data_which_panel_2015)
```
Now we check that everything is OK i.e. that the Ref,Name pairs are the same in both dataframes and ordered in the same way.
```{r}
  sum(data_which_panel_2014[,c("Ref")]!=data_2014[,c("Ref")])
...
Next let’s check differences in cadidate names
```{r}
  diff_names2015 <-as.character(data_which_panel_2015[,c("Name")])!=as.character(data_2015[,c("Name")])
  data.frame(data_which_panel_2015[diff_names2015,c("Name")],data_2015[diff_names2015,c("Name")])
```
So all is fine. Now we can change the levels of the panel to the standard form
```{r}
  levels(data_which_panel_2015$Panel)<- c("env","eng","sci","hum","med","nat","soc")
```
Finally… we can check that the panel information that we had already worked out was correct (for the cases where we had such info):
```{r}
  pos2015_panel_webfiles <- data_2015$Panel != "empty"
  setdiff(data_2015[pos2015_panel_webfiles,"Panel"],data_which_panel_2015[pos2015_panel_webfiles,"Panel"])
```
So now we can just safely replace the Panel column
```{r}
  setdiff(data_2015$Ref,data_which_panel_2015$Ref)
  data_2015$Panel <-data_which_panel_2015$Panel
  head(data_2015)
```
